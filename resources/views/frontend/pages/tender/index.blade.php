    ...
    <div class="latest-tenders">
      ...
      @foreach ($tenders as $tender)
        <div class="single-tender pointer">
          ...
          <p class="mt-2">
          <!-- <a href="{{ route('singleTender', $tender->slug) }}"> -->
          <h4>{{ $tender->title }}</h4></a></p>
          <div class="float-right">
            <a href="" class="btn btn-outline-secondary">Document Price: <span class="price">N{{ $tender->document_price }} </span></a>
            <a href="" class="ml-2 btn btn-outline-info">Security Amount: <span class="price">N{{ $tender->security_amount }} </span></a>
          </div>
          <div class="clearfix"></div>
          ...
          <a href="{{ route('user.tender.tenderApply', $tender->slug) }}" class="btn btn-sm btn-success float-right mt-3 mr-1">Evaluate <span class="badge badge-success">{{ \App\Models\TenderRequest::where('tender_id', $tender->id)->count() }}</span></a>
          <div class="clearfix"></div>
